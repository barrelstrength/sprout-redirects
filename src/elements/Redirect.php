...
namespace barrelstrength\sproutredirects\elements;

use barrelstrength\sproutredirects\elements\actions\ChangePermanentMethod;
use barrelstrength\sproutredirects\elements\actions\ChangeTemporaryMethod;
use barrelstrength\sproutredirects\enums\RedirectMethods;
use barrelstrength\sproutredirects\SproutRedirects;
use barrelstrength\sproutredirects\elements\db\RedirectQuery;
use barrelstrength\sproutredirects\records\Redirect as RedirectRecord;
use barrelstrength\sproutredirects\elements\actions\SetStatus;
use Craft;
...
/**
 * SproutRedirects - Redirect element type
 *
...
class Redirect extends Element
{
    ...
    {
        return Craft::t('sprout-redirects', 'Sprout SEO Redirects');
    }
    ...
    {
        $url = UrlHelper::cpUrl('sprout-redirects/redirects/edit/'.$this->id);
    ...
    {
        return [
            'oldUrl' => Craft::t('sprout-redirects', 'Old Url'),
            'newUrl' => Craft::t('sprout-redirects', 'New Url'),
            'method' => Craft::t('sprout-redirects', 'Method'),
            'count' => Craft::t('sprout-redirects', 'Count'),
            'test' => Craft::t('sprout-redirects', 'Test')
        ];
    }
    ...
    protected static function defineSortOptions(): array
    {
        $attributes = [
            ...
            'newUrl' => Craft::t('sprout-redirects', 'New Url'),
            'method' => Craft::t('sprout-redirects', 'Method'),
            'elements.dateCreated' => Craft::t('sprout-redirects', 'Date Created'),
            'elements.dateUpdated' => Craft::t('sprout-redirects', 'Date Updated'),
        ];
    ...
    protected static function defineSources(string $context = null): array
    {
        $sources = [
            [
                'key' => '*',
                'label' => Craft::t('sprout-redirects', 'All redirects'),
                'structureId' => SproutRedirects::$app->redirects->getStructureId(),
                'structureEditable' => true
            ]
        ];

        $methods = SproutRedirects::$app->redirects->getMethods();
    ...
    protected static function defineActions(string $source = null): array
    {
        ...
        $actions[] = Craft::$app->getElements()->createAction([
            'type' => Edit::class,
            'label' => Craft::t('sprout-redirects', 'Edit Redirect'),
        ]);

        // Change Permanent Method
        $actions[] = Craft::$app->getElements()->createAction([
            'type' => ChangePermanentMethod::class,
            'successMessage' => Craft::t('sprout-redirects', 'Redirects updated.'),
        ]);
        ...
        $actions[] = Craft::$app->getElements()->createAction([
            'type' => Delete::class,
            'confirmationMessage' => Craft::t('sprout-redirects', 'Are you sure you want to delete the selected redirects?'),
            'successMessage' => Craft::t('sprout-redirects', 'Redirects deleted.'),
        ]);
    ...
    {
        $methodOptions = SproutRedirects::$app->redirects->getMethods();
    ...
    public function beforeValidate()
    {
        if ($this->oldUrl && !$this->regex) {
            $this->oldUrl = SproutRedirects::$app->redirects->removeSlash($this->oldUrl);
        }

        if ($this->newUrl) {
            $this->newUrl = SproutRedirects::$app->redirects->removeSlash($this->newUrl);
