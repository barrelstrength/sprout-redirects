...
namespace barrelstrength\sproutredirects\controllers;
...
class RedirectsController extends Controller
{
    ...
    public function actionRedirectsIndexTemplate($siteHandle = null): Response
    {
        ...
        if (!$currentSite) {
            throw new ForbiddenHttpException(Craft::t('sprout-redirects', 'Something went wrong'));
        }

        return $this->renderTemplate('sprout-redirects/redirects', [
    ...
    public function actionEditRedirect($redirectId = null, $siteHandle = null, Redirect $redirect = null)
    {
        ...
        if (!$currentSite) {
            throw new ForbiddenHttpException(Craft::t('sprout-redirects', 'Unable to identify current site.'));
        }
        ...
        if ($redirect === null) {
            if ($redirectId !== null) {
                ...
                if (!$redirect) {
                    throw new NotFoundHttpException(Craft::t('sprout-redirects', 'Unable to find a Redirect with the given id: {id}', [
                        'id' => $redirectId
                    ]));
                }
        ...
        $continueEditingUrl = 'sprout-redirects/redirects/edit/{id}/'.$currentSite->handle;
        $saveAsNewUrl = 'sprout-redirects/redirects/new/'.$currentSite->handle;
    ...
    public function actionSaveRedirect()
    {
        ...
        if ($redirectId) {
            ...
            if (!$redirect) {
                throw new Exception(Craft::t('sprout-redirects', 'No redirect exists with the ID “{id}”', [
                    'id' => $redirectId
                ]));
            }
        ...
        if (!Craft::$app->elements->saveElement($redirect, true)) {
            Craft::$app->getSession()->setError(Craft::t('sprout-redirects', 'Couldn’t save redirect.'));
        ...
        Craft::$app->getSession()->setNotice(Craft::t('sprout-redirects', 'Redirect saved.'));
    ...
    public function actionDeleteRedirect()
    {
        ...
        if (Craft::$app->elements->deleteElementById($redirectId)) {
            Craft::$app->getSession()->setNotice(Craft::t('sprout-redirects', 'Redirect deleted.'));
            $this->redirectToPostedUrl();
        } else {
            Craft::$app->getSession()->setError(Craft::t('sprout-redirects', 'Couldn’t delete redirect.'));
        }
    }
}
